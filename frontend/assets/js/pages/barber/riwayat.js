/**
 * pages/barber/riwayat.js
 * Riwayat transaksi Barber dengan filter tanggal, pagination, dan cetak ulang struk.
 */
...
export async function renderBarberRiwayat(root) {
  root.innerHTML = `
    <div class="mx-auto max-w-4xl space-y-4">
      <div class="glass-card flex flex-wrap items-end gap-3 p-4">
        ...
        <button id="filter-apply" type="button" class="btn-barber">Terapkan Filter</button>
      </div>
  ...
  async function load() {
    try {
      const result = await apiCall('barberListTransaksi', {
      ...
      if (result.transaksi.length === 0) {
        ...
        tbody.innerHTML = result.transaksi.map((t) => `
          <tr class="border-t border-slate-200/60 dark:border-white/10">
            ...
            <td class="px-3 py-2">${t.NamaPelanggan}</td>
            <td class="px-3 py-2">${t.NamaCapster}</td>
            <td class="px-3 py-2 font-semibold">${formatRupiah(t.GrandTotal)}</td>
            <td class="px-3 py-2">
              <span class="rounded-full px-2 py-0.5 text-xs font-medium ${t.Status === 'Selesai' ? 'bg-gabungan-100 text-gabungan-700 dark:bg-gabungan-500/10 dark:text-gabungan-300' : 'bg-red-100 text-red-700 dark:bg-red-500/10 dark:text-red-300'}">${t.Status}</span>
            </td>
        ...
        tbody.querySelectorAll('.print-btn').forEach((btn) => {
          btn.addEventListener('click', async () => {
            const { transaksi } = await apiCall('barberGetTransaksi', { id: btn.dataset.id });
            openStrukModal(transaksi);
          });
        });
      }
