/**
 * dashboard/warkop — Dashboard Warkop: kartu Hari Ini/Bulan Ini tetap +
 * Periode Terpilih (filter) + Menu/Kategori Terlaris + grafik Pendapatan,
 * Produk Terjual, Metode Pembayaran (semua mengikuti filter).
 */
...
function terlarisCard(title, item, nameKey, qtyLabel) {
  if (!item) {
  ...
  return `
    <div class="glass-card p-4">
      ...
      <div class="mt-1 text-lg font-bold text-warkop-600 dark:text-warkop-400">${escapeHtml(item[nameKey])}</div>
      <div class="text-sm text-slate-500 dark:text-slate-400">${item.totalQty} ${qtyLabel} · ${formatRupiah(item.totalPendapatan)}</div>
    </div>
  `;
}

export async function renderDashboardWarkop(container) {
  container.innerHTML = `
    <div class="mx-auto max-w-5xl">
      <h1 class="mb-4 text-xl font-bold">Dashboard Warkop</h1>
      ...
      <div class="grid gap-6 lg:grid-cols-3">
        ...
        <div class="glass-card p-4 lg:col-span-3">
          <h3 class="mb-3 text-sm font-semibold text-slate-500 dark:text-slate-400">Produk Terjual per Hari</h3>
          <canvas id="chart-produk" height="160"></canvas>
        </div>
      </div>
    </div>
  `;
  ...
  function renderFixedCards(data) {
    ...
    fixedCardsEl.innerHTML =
      statSection('Hari Ini', [
        statTile('Pendapatan', hi.totalPendapatan, { tone: 'warkop' }),
        ...
        statTile('Laba Bersih', hi.labaBersih, { tone: 'warkop' })
      ], 'warkop') +
      statSection('Bulan Ini', [
        statTile('Pendapatan', bi.totalPendapatan, { tone: 'warkop' }),
        ...
        statTile('Laba Bersih', bi.labaBersih, { tone: 'warkop' })
      ], 'warkop');
  }
  ...
  function renderPeriodeCards(data) {
    ...
    periodeCardsEl.innerHTML = statSection('Periode Terpilih', [
      statTile('Pendapatan', p.totalPendapatan, { tone: 'warkop' }),
      ...
      statTile('Laba Bersih', p.labaBersih, { tone: 'warkop' })
    ], 'warkop');
  }
  ...
  async function loadAndRender(filterState) {
    let data;
    try {
      data = await apiCall('dashboard.view', { dashboard: 'warkop', ...filterState });
    } catch (err) {
    ...
    terlarisEl.innerHTML =
      terlarisCard('Menu Terlaris (bulan ini)', data.menuTerlaris[0], 'nama', 'terjual') +
      terlarisCard('Kategori Terlaris (bulan ini)', data.kategoriTerlaris[0], 'kategori', 'terjual');

    await renderLineChart(container.querySelector('#chart-pendapatan'), data.chartPendapatan, { granularity: 'day', color: 'warkop', label: 'Pendapatan' });
    await renderBarChart(container.querySelector('#chart-produk'), data.chartProdukTerjual, { granularity: 'day', color: 'warkop', label: 'Terjual' });
